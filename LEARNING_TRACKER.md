...
- ⚠️ BUG surfaced (pre-existing, not migration): `GET /movies/:id` no-token → 500 not 401 — `authMiddelWare` reads `req.cookies.jwt` but no cookie-parser mounted → `req.cookies` undefined → TypeError. Fix: `bun add cookie-parser` + `app.use(cookieParser())` (cookie-auth path is intended — generateToken sets `res.cookie("jwt")`)
