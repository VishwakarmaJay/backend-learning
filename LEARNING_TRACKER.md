...
**Current topic:** ✅ P.0 TS migration · ✅ P.1 advanced queries · 🔄 P.2 Repository/Service (core done; tests + DTOs deferred) · ✅ P.3 Pagination & filtering (offset + cursor + filters/search, all measured) → **next: P.4 Phase P checkpoint** (or clear P.2/P.3 cleanup debt)  
...
- [x] Migrate leaf-first: **utils ✓ · config ✓ · models ✓ · middleware ✓ · controllers ✓ · routes ✓ · server ✓** — FULL tree on TS, `tsc` 0 errors, boots under Bun, /api-docs 200
- HTTP-layer typing done: `Request` augmented with `user` (`src/types/express.d.ts`); `jwt.verify` cast to `{id:number}`; `generateToken(userId:number)`; model fields camelCase (releaseYear/posterUrl) to match seed/controllers/OpenAPI/DB; `@types/swagger-ui-express` + `@types/express` + `@types/nodemailer` added
- ✅ FIXED (2026-09-02): `GET /movies/:id` no-token → was **500** — `authMiddelWare` read `req.cookies.jwt` but no cookie-parser → `req.cookies` undefined → TypeError. Fixed by adding `cookie-parser` + `app.use(cookieParser())` (before routes/auth); verified **401**. Also enables the intended cookie-auth path (`generateToken` sets `res.cookie("jwt")`). This was the latent bug flagged in the very first review of the project.
- 🔥 Mistake (pattern): blind case-insensitive `.js`→`` find-replace to strip import extensions ate `.json`/`.JSON` → 11 casualties (`res….json(`→`on(`, `express.json`→`expresson`, `DataTypes.JSON`→`DataTypesON`). Under bundler resolution, stripping extensions was UNNECESSARY (`.js` and extensionless both resolve). Never blind-replace a bare `.js`.
...
- [x] Raw queries — v7 `connection.query(sql)` → `[rows, metadata]`; **parameterize** with `{ replacements: { email } }` (never string-concat → SQL injection); bypasses the ORM (no scopes/hooks/types — a raw `SELECT *` ignores the password default-scope).
...
- [x] EXPLAIN + indexes — MySQL 8 tree plans via raw `EXPLAIN`. Read it: **"Table scan"** 🚩 (reads every row) vs **"Index lookup"** / **"Rows fetched before execution"** (unique index → const single-row lookup). Indexes: PK/unique/FK are all indexes; **leftmost-prefix rule** (composite `(userId,movieId)` helps `userId` but not `movieId` alone → FK index covers movieId); trade-off = faster reads, slower writes. **Applied:** `users.email` had NO index → login = full table scan → added `@Unique` on email → const lookup + duplicate emails rejected.
...
**Status:** `COMPLETE` (2026-09-17) — offset + cursor + dynamic filters/search all built on `GET /movies`, verified against a 131k-row table.
...
- [x] Dynamic filters — repo builds a `where` conditionally from an optional `filters` object (only present params added): `year` exact, `runtime` range. **Same-column merge trap:** min+max both target `runtime` → a second `where.runtime = {...}` clobbers the first → merge with `...(cond && { [Op.gte]: v })` spread idiom (spreads the pair when true, nothing when false). Multiple keys AND together automatically. Empty filters → `where={}` → returns all (bare list still works).
- [x] Search — `q` → `{ title: { [Op.like]: `%${q}%` } }`. **Injection-safe (proven empirically):** `q="' OR '1'='1"` returned 0 rows, not all — Sequelize sends `where` values as bound params (`LIKE ?`), so user input is data, never SQL. Contrast raw `connection.query` (must use `replacements`). Optional-param gotcha: an absent filter means "don't filter," never a required param / bare `return` (that hangs the response).
...
**Debt (P.3 cleanup):** `filters` type is a `declare global` in `src/types/express.d.ts` (wrong file + global pollution) → move to an exported `MovieFilters`, PascalCase, imported explicitly; `where: any` → `WhereOptions<Movie>`. `cusor` typo still in the public query param `?cusor=` + `getMovieCusorList`/`movieCusorList` names (internals fixed) — rename to `cursor`. Pattern: stray autocomplete imports keep appearing (`number` from zod, `hsts` from helmet — both since removed) → glance at the import block before saving. `findPage`/cursor only on movies; watchlist list still unpaginated.
